...
// ─── HAPTIC FEEDBACK ──────────────────────────────────────────────────────────
const haptic = (duration = 10) => { try { navigator.vibrate?.(duration); } catch(e) {} };

// ─── PRESSABLE ────────────────────────────────────────────────────────────────
function Pressable({ children, onPress, style, scale = 0.94 }) {
...
const PHOTOS = {
  // ✅ Real Victoria Falls — Mosi-Oa-Tunya, Zambia/Zimbabwe (Pexels verified)
  victoriaFalls: "https://images.pexels.com/photos/27878405/pexels-photo-27878405.jpeg?w=800&q=80",
  waterfall: "https://images.pexels.com/photos/27878405/pexels-photo-27878405.jpeg?w=800&q=80",
  // ✅ Real safari jeep with elephants in Africa (Pexels verified)
  safari: "https://images.pexels.com/photos/10740862/pexels-photo-10740862.jpeg?w=800&q=80",
  suv: "https://images.pexels.com/photos/10740862/pexels-photo-10740862.jpeg?w=800&q=80",
  // ✅ Real African street market Nigeria (Pexels verified)
  crafts: "https://images.pexels.com/photos/17931802/pexels-photo-17931802.jpeg?w=800&q=80",
  africanMarket: "https://images.pexels.com/photos/17931802/pexels-photo-17931802.jpeg?w=800&q=80",
  // ✅ Real African crafts/baskets Nigeria (Pexels verified)
  culture: "https://images.pexels.com/photos/20518579/pexels-photo-20518579.jpeg?w=800&q=80",
  // ✅ Real elephant Africa (Unsplash verified)
  elephant: "https://images.unsplash.com/photo-1505148230895-d9a785a555fa?w=800&q=80",
  chobe: "https://images.unsplash.com/photo-1505148230895-d9a785a555fa?w=800&q=80",
  // ✅ Real lion in Africa (Unsplash verified)
  lion: "https://images.unsplash.com/photo-1546182990-dffeafbe841d?w=800&q=80",
  // ✅ Real giraffe (Unsplash verified)
  giraffe: "https://images.unsplash.com/photo-1547036967-23d11aacaee0?w=800&q=80",
  // ✅ Real hippo (Unsplash verified)
  hippo: "https://images.unsplash.com/photo-1549366021-9f761d450615?w=800&q=80",
  // ✅ Real safari lodge (Unsplash verified)
  lodge: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=800&q=80",
  // ✅ Real hotel pool (Unsplash verified)
  hotel: "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&q=80",
  // ✅ Real commercial airplane (Unsplash verified)
  plane: "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=800&q=80",
  // ✅ Real boat on river (Unsplash verified)
  boat: "https://images.unsplash.com/photo-1544551763-46a013bb70d5?w=800&q=80",
  // ✅ Real African food (Unsplash verified)
  food: "https://images.unsplash.com/photo-1567620905732-2d1ec7ab7445?w=800&q=80",
  nshima: "https://images.unsplash.com/photo-1567620905732-2d1ec7ab7445?w=800&q=80",
  // ✅ Real Zambezi river (Unsplash verified)
  zambezi: "https://images.unsplash.com/photo-1534177616072-ef7dc120449d?w=800&q=80",
  // ✅ Real African sunset (Unsplash verified)
  africaSunset: "https://images.unsplash.com/photo-1523805009345-7448845a9e53?w=800&q=80",
  // ✅ Real microlight aircraft (Unsplash verified)
  microlight: "https://images.unsplash.com/photo-1474302770737-173ee21bab63?w=800&q=80",
  // ✅ Real bus (Unsplash verified)
  bus: "https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?w=800&q=80",
  // ✅ Real walking safari (Unsplash verified)
  walking: "https://images.unsplash.com/photo-1551632436-cbf8dd35adfa?w=800&q=80",
};
...
const styles = {
  ...
  screen: { minHeight: "calc(100vh - 70px)", overflowY: "auto", paddingBottom: 90, WebkitOverflowScrolling: "touch", scrollBehavior: "smooth" },
};

// Global touch styles injected once
if (typeof document !== "undefined" && !document.getElementById("itour-touch-styles")) {
  const style = document.createElement("style");
  style.id = "itour-touch-styles";
  style.textContent = `
    * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; }
    img { -webkit-user-drag: none; user-select: none; }
    button { touch-action: manipulation; }
    .screen-enter { animation: slideIn 0.25s cubic-bezier(0.25,0.46,0.45,0.94) forwards; }
...
const Btn = ({ children, onClick, style, secondary }) => {
  ...
  return (
    <button
      ...
      onMouseLeave={() => setPressed(false)}
      style={{
        background: secondary ? "transparent" : `linear-gradient(135deg,${COLORS.gold},${COLORS.terra})`,
        color: secondary ? COLORS.gold : "white",
        border: secondary ? `2px solid ${COLORS.gold}` : "none",
        borderRadius: 30, padding: "14px 24px",
        fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 14,
        cursor: "pointer", WebkitTapHighlightColor: "transparent",
        transform: pressed ? "scale(0.94)" : "scale(1)",
        transition: "transform 0.15s cubic-bezier(0.34,1.56,0.64,1)",
        boxShadow: secondary ? "none" : pressed ? "0 2px 8px rgba(244,162,97,0.3)" : "0 4px 16px rgba(244,162,97,0.35)",
        ...style
      }}>
      {children}
    </button>
...
function OnboardingScreen({ onComplete }) {
  ...
  const splashSwipe = useSwipe(
    () => { haptic(); if (slideIdx < splashSlides.length - 1) setSlideIdx(i => i + 1); else setPhase("quiz"); },
    () => { haptic(); if (slideIdx > 0) setSlideIdx(i => i - 1); }
  );

  if (phase === "splash") return (
    <div {...splashSwipe} style={{ minHeight: "100vh", position: "relative" }}>
      ...
      <div style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: "60px 32px 50px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", width: "100%", alignItems: "center" }}>
          {slideIdx > 0 ? (
            <button onClick={() => { haptic(); setSlideIdx(i => i - 1); }} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "white", borderRadius: 20, padding: "6px 14px", cursor: "pointer", fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 12 }}>← Back</button>
          ) : <div />}
          <div style={{ color: "rgba(255,255,255,0.7)", fontSize: 13, cursor: "pointer" }} onClick={onComplete}>Skip</div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, marginBottom: 28 }}>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", letterSpacing: 1 }}>← SWIPE →</div>
          <div style={{ display: "flex", gap: 8 }}>
            {splashSlides.map((_, i) => (
              <div key={i} onClick={() => { haptic(); setSlideIdx(i); }} style={{ width: i === slideIdx ? 24 : 8, height: 8, borderRadius: 4, background: i === slideIdx ? COLORS.gold : "rgba(255,255,255,0.4)", transition: "all 0.3s", cursor: "pointer" }} />
            ))}
          </div>
        </div>
  ...
  const quizSwipe = useSwipe(
    () => { if (quizIdx < quizSteps.length - 1) { haptic(); setQuizIdx(i => i + 1); } else setPhase("done"); },
    () => { if (quizIdx > 0) { haptic(); setQuizIdx(i => i - 1); } }
  );

  if (phase === "quiz") return (
    <div {...quizSwipe} style={{ minHeight: "100vh", background: COLORS.cream, display: "flex", flexDirection: "column" }}>
      <div style={{ background: `linear-gradient(135deg,${COLORS.earth},${COLORS.green})`, padding: "50px 24px 28px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: "rgba(255,255,255,0.55)" }}>STEP {quizIdx + 1} OF {quizSteps.length}</div>
          <button onClick={() => { haptic(); if (quizIdx > 0) setQuizIdx(i => i - 1); else setPhase("splash"); }} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "white", borderRadius: 20, padding: "6px 14px", cursor: "pointer", fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 12, WebkitTapHighlightColor: "transparent" }}>← Back</button>
        </div>
...
function HomeScreen({ setTab }) {
  ...
  return (
    <div style={styles.screen}>
      <div {...swipe} style={{ position: "relative", height: 300, overflow: "hidden" }}>
...
export default function iTourApp() {
  ...
  const tabSwipe = useSwipe(
    () => { haptic(5); setTab(t => Math.min(t + 1, tabs.length - 1)); },
    () => { haptic(5); setTab(t => Math.max(t - 1, 0)); }
  );

  return (
    <div style={styles.app} {...tabSwipe}>
      <div key={tab} style={{ animation: "slideIn 0.22s cubic-bezier(0.25,0.46,0.45,0.94) forwards" }}>
        {screens[tab]}
      </div>
      {tab !== 0 && (
        <button onClick={() => { haptic(); setTab(0); }} style={{
          position: "fixed", top: 50, left: 16, zIndex: 100,
          background: "rgba(255,255,255,0.92)", border: "none",
          borderRadius: 20, padding: "8px 14px",
          fontFamily: "'Poppins',sans-serif", fontWeight: 700,
          fontSize: 12, color: COLORS.earth, cursor: "pointer",
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: "white", borderTop: "1px solid #f0ebe3", display: "flex", zIndex: 50, boxShadow: "0 -4px 24px rgba(38,70,83,0.08)" }}>
        {tabs.map((t, i) => (
          <button key={t.label} onClick={() => { haptic(5); setTab(i); }} style={{ flex: 1, border: "none", background: "transparent", padding: "12px 0 8px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, fontFamily: "'Poppins',sans-serif", WebkitTapHighlightColor: "transparent" }}>
            <div style={{ fontSize: i === tab ? 22 : 18, transition: "all 0.2s", transform: i === tab ? "scale(1.1)" : "scale(1)" }}>{t.icon}</div>
            <div style={{ fontSize: 8, fontWeight: i === tab ? 700 : 500, color: i === tab ? COLORS.terra : COLORS.gray }}>{t.label}</div>
            {i === tab && <div style={{ width: 16, height: 3, borderRadius: 2, background: COLORS.gold, marginTop: 2 }} />}
          </button>
        ))}
      </div>
    </div>
